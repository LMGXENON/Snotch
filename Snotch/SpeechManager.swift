...
final class SpeechManager: NSObject, ObservableObject {
    ...
    private func processBuffer(_ buffer: AVAudioPCMBuffer) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let adaptive = max(self.rmsThreshold, min(0.0020, self.noiseFloorRMS * 1.18 + 0.00003))
            if rms > adaptive || self.audioLevel > 0.02 {
    ...
    private func handleVoiceBurst(rms: Float) {
        ...
        // Tick at 20 Hz. Rate = (wordsPerMinute / 60) words/sec ÷ avgWordsPerLine → lines/sec.
        // scrollSpeed is a user multiplier on top of that natural reading pace.
        // e.g. 130 WPM, 8 words/line, speed=1.0 → ~0.271 lines/sec → 1 line per ~3.7 s.
