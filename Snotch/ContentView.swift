...
struct ContentView: View {
    ...
    @State private var recentScriptIDs: [UUID] = []
            ...
            if !recentScripts.isEmpty {
                VStack(alignment: .leading, spacing: 5) {
                    Text("Recent")
                        ...
                        .foregroundColor(Color(white: pillLight ? 0.42 : 0.40))
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 6) {
                            ForEach(recentScripts, id: \.id) { script in
                                Button(action: { select(script) }) {
                                    Text(script.title)
                                        .font(.system(size: 9.5, weight: .medium))
                                        .lineLimit(1)
                                        .foregroundColor(Color(white: pillLight ? 0.24 : 0.70))
    ...
    private var recentScripts: [SnotchScript] {
        recentScriptIDs
            .compactMap { id in store.scripts.first(where: { $0.id == id }) }
            .prefix(6)
            .map { $0 }
    }

    private func loadRecents() {
        let values = UserDefaults.standard.array(forKey: "snotch.recents") as? [String] ?? []
        recentScriptIDs = values.compactMap(UUID.init(uuidString:))
    }

    private func saveRecent(_ id: UUID) {
        recentScriptIDs.removeAll(where: { $0 == id })
        recentScriptIDs.insert(id, at: 0)
        if recentScriptIDs.count > 20 {
            recentScriptIDs.removeLast(recentScriptIDs.count - 20)
        }
        UserDefaults.standard.set(recentScriptIDs.map(\.uuidString), forKey: "snotch.recents")
    }
